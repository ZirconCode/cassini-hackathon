...
class GreenMetric:
    ...
    def get_metric(self, lat, lon):
        if self.m_xmap_cood is None:
            self.create_maps()
        if (
            (lat < self.m_bounds["south"])
            or (lat > self.m_bounds["north"])
            or (lon < self.m_bounds["west"])
            or (lon > self.m_bounds["east"])
        ):
    ...
    def green_distance_index(
        ...
        ndv_reduce = block_reduce(
            ndv_index, block_size=(reduce_factor, reduce_factor), func=np.mean, cval=0
        )
        reduce_pos_x = np.transpose(
            np.tile(
                np.linspace(
                    start=0 + reduce_factor / 2,
    ...
    def load_streets(
        self, streets: np.ndarray, x_lims: Tuple[int, int], y_lims: Tuple[int, int]
    ):
        ...
        streets = crop_layer(image=[streets], x_lims=x_lims, y_lims=y_lims)[0]

        self.m_streets = streets
        self.m_street_locs = list(
            zip(np.where(streets == 1)[1], np.where(streets == 1)[0])
        )
    ...
    def save_metric(self, output_path: str):
        np.save(file=output_path, arr=self.m_metric)

    def load_metric(self, input_path, resolution):
    ...
    def check_resolution(self, image):
        if self.m_resolution is not None:
            if (image.shape[0] != self.m_resolution[0]) or (
                image.shape[1] != self.m_resolution[1]
            ):
    ...
    def create_maps(self):
        self.m_xmap = np.transpose(
            np.tile(
                np.arange(-self.m_resolution[1] / 2, self.m_resolution[1] / 2),
                (self.m_resolution[0], 1),
            )
        )
        self.m_ymap = np.tile(
            np.arange(-self.m_resolution[0] / 2, self.m_resolution[0] / 2),
            (self.m_resolution[1], 1),
        )

        ew_pix = (self.m_bounds["east"] - self.m_bounds["west"]) / self.m_resolution[1]
        ns_pix = (self.m_bounds["north"] - self.m_bounds["south"]) / self.m_resolution[
            0
        ]
        ...
        self.m_xmap_cood = np.linspace(
            start=self.m_bounds["west"] + ew_pix / 2,
